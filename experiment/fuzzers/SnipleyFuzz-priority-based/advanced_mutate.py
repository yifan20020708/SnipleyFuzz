...
def advanced_mutate(seeds: list[Seed], interact: Messenger):
    ...
    while True:
        ...
        seed_scores = []
        for seed in seeds:
            # capability = α * div + β * NRS_cum
            capability = cfg.ALPHA_CAPABILITY_SEED * seed.response_div + cfg.BETA_CAPABILITY_SEED * seed.nrs_cum
            redundancy_factor = 1 - getattr(seed, 'redundancy_score', 0.0)
            if redundancy_factor < 0:
                redundancy_factor = 0.0
            fresh_factor = 1 + cfg.FRESH_WEIGHT_SEED * seed.interval
            use_factor = 1 / (1 + cfg.USE_WEIGHT_SEED * seed.number_used)
            P_seed = capability * redundancy_factor * fresh_factor * use_factor  # formula (3)
            seed_scores.append(P_seed)
        seed_probs = _normalize_with_min_prob(seed_scores, cfg.LEAST_PROB_SEED)
        seed_index = np.random.choice(len(seeds), p=seed_probs)
        selected_seed = seeds[seed_index]
        # Update seed usage stats
        selected_seed.number_used += 1
        selected_seed.interval = 0
        
        # Message selection within selected seed (similar to seed selection)
        msg_scores = []
        for msg in selected_seed.M:
            capability = cfg.ALPHA_CAPABILITY_MSG * msg.response_div + cfg.BETA_CAPABILITY_MSG * msg.nrs_cum
            redundancy_factor = 1 - getattr(msg, 'redundancy_score', 0.0)
            if redundancy_factor < 0:
                redundancy_factor = 0.0
            fresh_factor = 1 + cfg.FRESH_WEIGHT_MSG * msg.interval
            use_factor = 1 / (1 + cfg.USE_WEIGHT_MSG * msg.number_used)
            P_msg = capability * redundancy_factor * fresh_factor * use_factor
            msg_scores.append(P_msg)
        msg_probs = _normalize_with_min_prob(msg_scores, cfg.LEAST_PROB_MSG)
        msg_index = np.random.choice(len(selected_seed.M), p=msg_probs)
        selected_msg = selected_seed.M[msg_index]
        # Update message usage stats
        selected_msg.number_used += 1
        selected_msg.interval = 0
        ...
        for cluster in cluster_levels:
            novelty_factor = cfg.ALPHA_CAPABILITY_CLUSTER * cluster.interested
            # Fresh(Ck) ~ (rounds since last used)
            interval = (current_round - cluster.last_used_round) if cluster.last_used_round >= 0 else current_round
            fresh_factor = 1 + interval * cfg.FRESH_WEIGHT_CLUSTER
            use_factor = 1 / (1 + cfg.USE_WEIGHT_CLUSTER * cluster.number_used)
            P_cluster = novelty_factor * fresh_factor * use_factor
            cluster_scores.append(P_cluster)
        cluster_probs = _normalize_with_min_prob(cluster_scores, cfg.LEAST_PROB_CLUSTER)
        cluster_index = np.random.choice(len(cluster_levels), p=cluster_probs)
        selected_cluster = cluster_levels[cluster_index]
        # Update cluster usage stats
        selected_cluster.number_used += 1
        selected_cluster.last_used_round = current_round
        ...
        k = min(2, len(St_snippets))
        selected_snippets = random.sample(St_snippets, k=k)
        ...
        for i, msg in enumerate(seed_sequence):
            if i == mutated_msg_index:
                ...
                mutated_msg.raw["Content"] = mutated_bytes
