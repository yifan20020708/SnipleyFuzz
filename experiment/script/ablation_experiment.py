    ...
    for file in python_files[index]:
        ...
        while(1):
            if os.path.exists(folder + file_name):
                ...
                continue
        record[file.split("/")[-1].split("IoTFuzz_")[-1].split(".")[0]] = info_single
        print(file.split("/")[-1].split("IoTFuzz_")[-1].split(".")[0] + "'information has been recorded!")
    ...
    while(1):
        ...
        for i in range(len(crash_number)):
            if crash_number[i] == 1:
                ...
                crash_time_single[python_files[index][i].split("/")[-1].split("IoTFuzz_")[-1].split(".")[0]] = (finish_time - start_time) / 3600
...
def main():
    ...
    for i in range(len(python_files)):
        ...
        for file in python_files[i]:
            ...
            if "yeelight" in file_name:
                yeelight_information["path"][file.split("/")[-1].split("IoTFuzz_")[-1].split(".")[0]] = []
                yeelight_information["seed"][file.split("/")[-1].split("IoTFuzz_")[-1].split(".")[0]] = []
                yeelight_information["number"][file.split("/")[-1].split("IoTFuzz_")[-1].split(".")[0]] = []
            if "xiaomi_plug" in file_name:
                xiaomi_plug_information["path"][file.split("/")[-1].split("IoTFuzz_")[-1].split(".")[0]] = []
                xiaomi_plug_information["seed"][file.split("/")[-1].split("IoTFuzz_")[-1].split(".")[0]] = []
                xiaomi_plug_information["number"][file.split("/")[-1].split("IoTFuzz_")[-1].split(".")[0]] = []
            if "xiaomi_camera" in file_name:
                xiaomi_camera_information["path"][file.split("/")[-1].split("IoTFuzz_")[-1].split(".")[0]] = []
                xiaomi_camera_information["seed"][file.split("/")[-1].split("IoTFuzz_")[-1].split(".")[0]] = []
                xiaomi_camera_information["number"][file.split("/")[-1].split("IoTFuzz_")[-1].split(".")[0]] = []
            if os.path.exists(folder + file_name):
    ...
    for i in range(len(all_information)):
        ...
        if len(single_information["crash"]) == 3:
        ...
        # generate a file
        path_file = "./output/ablation/file-one/"
        file_name_single = "ablation_experiment_" + labels[i] + ".txt"
        ...
        for key in list(single_information["path"].keys()):
            if "-M-SN" in key:
                plt.plot(time_points, single_information["path"][key], label = labels[i] + "-M-SN", color = 'blue')
            elif "-M-S" in key:
                plt.plot(time_points, single_information["path"][key], label = labels[i] + "-M-S", color = 'green')
            else:
        ...
        for key in list(single_information["crash"]):
            if "-M-SN" in key:
                ...
                plt.axvline(x = single_information["crash"][key], linestyle='--', color='green', linewidth=0.5)
            else:
        ...
        figure_name = "ablation_experiment_path_" + labels[i] + ".pdf"
        ...
        for key in list(single_information["seed"].keys()):
            if "-M-SN" in key:
                plt.plot(time_points, single_information["seed"][key], label = labels[i] + "-M-SN", color = 'blue')
            elif "-M-S" in key:
                plt.plot(time_points, single_information["seed"][key], label = labels[i] + "-M-S", color = 'green')
            else:
        ...
        figure_name = "ablation_experiment_seed_" + labels[i] + ".pdf"
        plt.savefig(path + figure_name)
        plt.close()
        ...
        for key in list(single_information["number"].keys()):
            if "-M-SN" in key:
                plt.plot(time_points, single_information["number"][key], label = labels[i] + "-M-SN", color = 'blue')
            elif "-M-S" in key:
                plt.plot(time_points, single_information["number"][key], label = labels[i] + "-M-S", color = 'green')
            else:
